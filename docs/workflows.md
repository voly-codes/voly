...
Workflow в CodeOps — конечный автомат, управляющий выполнением задачи от начала до конца. Каждый Workflow состоит из упорядоченных шагов (`WorkflowStep`), объединённых в объекте `WorkflowDefinition`.

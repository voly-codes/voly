...
### Decision (reuses `voly/plan/` — no new schema)

An approved-for-review Option becomes a **one-step Plan**:
...
This is the exact `pending → running → done → verifying → verified` FSM
from `docs/proposals/plan-gate-verification.md`, with `human_review` added
to the acceptance-check table as a check that only resolves via explicit
`POST /api/decisions/{plan_id}/feedback` (approve/reject) — mirroring the
existing `POST /api/evidence/{task_id}/feedback` pattern used for
security/testing `review_required` tasks today.
...
| `voly/capability/` (`ExecutorMatcher`, `fallback.py`) | Route/score business executors alongside code executors | register new capability domain, e.g. `CapabilityDomain.BUSINESS_ACTION` |
...
- New package `voly/dspy/programs/analyst.py` — one file, existing
  programs directory.
- `voly/plan/` gets two additive changes (`mode: business`,
  `human_review` check), not a rewrite.
...
| `voly/plan/verify.py` | add `human_review` acceptance check (resolves only via explicit feedback endpoint, like existing security/testing `review_required`) |
...
**Done when:** an Option above threshold produces a Plan stuck at
`verifying` until an explicit approve/reject call moves it to
`verified`/`failed` — the human "yes/no" checkpoint from Ismail's model,
running on the FSM that already ships in `voly/plan/`.
...
| Deliverable | Detail |
|---|---|
| `voly/executor/http_action.py` | generic allowlisted HTTP action executor, implements `Executor`/`ExecutorResult` |
| `voly/executor/notify.py` | send email/Slack via existing outbound channels |
| `voly/capability/` | register `CapabilityDomain.BUSINESS_ACTION`; seed profiles in `capability/seeds/` |
...
**Done when:** an approved (`verified`) business Plan step runs through a
real `Executor`, produces an `EvidenceRecord`, and emits a `TaskEvent` —
same telemetry contract a code task gets today.
...
6. **Human checkpoint is mandatory in `active` mode** — a business Plan
   cannot reach `verified` without an explicit approve call, exactly as
   `review_required` already works for security/testing tasks.
...
```text
[ ] PR0  docs/proposals/business-ooda-loop.md + ARCHITECTURE pointer
[ ] PR1  voly/sensing/{schema,store,connectors/{base,rss}}.py + tests
[ ] PR2  voly/dspy/programs/analyst.py + voly/sensing/interpret.py + tests
[ ] PR3  plan/ mode=business + human_review check + decisions API/CLI/UI

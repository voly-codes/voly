...
ba50121 — Implemented `Agent(tools=[...])` (bounded tool-call loop against an explicit allowlist registry, `voly/sdk/tools.py`), `Agent(output_schema=...)` (prompt-based structured-output validation, pydantic model or raw JSON schema), and `voly/capability/routing.py::capability_route()` (evidence-based model/executor auto-selection). Found and fixed while wiring capability routing: `Agent`'s own chat/executor resolution is a separate code path from `PlanRunner`'s default `_exec_chat`/`_exec_executor` — a `Workflow`-compiled node never calls `Agent`'s methods, so routing had to be wired into both, or every `Workflow`/hand-written-Plan node would have stayed on static resolution forever regardless of the new `Agent`-level support.

4d8afd6 — Added Context7 (`@upstash/context7-mcp`) to `MCPManager.BUILTIN_SERVERS` and `voly mcp config`'s default set — pulls up-to-date, version-specific library docs into the prompt, reducing an executor hallucinating an outdated API. Verified live (`npx -y @upstash/context7-mcp` starts and identifies itself). Fixed a latent bug found while touching `register_builtin()`: it indexed `spec["env"]` directly, which would have raised `KeyError` the first time anything called `register_builtin("docker")` — the only `BUILTIN_SERVERS` entry with no `"env"` key at all; never caught before because `voly mcp config`'s hardcoded default list excludes `docker`.

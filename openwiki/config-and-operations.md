...
- post-run evaluation is staged through `evaluation.enabled`;
  ...
  `evaluation.llm_judge.mode` remains `off` unless bounded task/result text may
  explicitly be sent to a configured model in `shadow` or `required` mode

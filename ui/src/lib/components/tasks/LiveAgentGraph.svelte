<script>
  ...
  import { cancelRun } from '../../api/client.js'
  ...
  async function requestCancel() {
    if (!task?.task_id || cancelling || cancelRequested || task.cancel_requested) return
    cancelling = true
    try {
      await cancelRun(task.task_id)
...
<section class="agent-flow">
  <header class="flow-head">
    ...
    <div class="run-state">
      ...
      {#if task?.workflow && task?.status === 'running'}
        <button type="button" disabled={cancelling || cancelRequested || task.cancel_requested} onclick={requestCancel}>
          {cancelRequested || task.cancel_requested ? 'cancel queued' : 'cancel'}
        </button>
      {/if}
    </div>
  </header>

  {#if task?.live_steps?.length}
    <div class="live-steps" aria-label="Live work stages">
      {#each task.live_steps as step}
        <span class="live-step status-{step.status || 'pending'}">
          <i></i>{step.role || step.id} · {step.status || 'pending'}
        </span>
      {/each}
    </div>
  {/if}
    ...
    <div class="canvas" style:width={`${layout.width}px`} style:height={`${layout.height}px`}>
      ...
      {#each layout.nodes as node (node.id)}
        ...
        >
          ...
          {#if node.skills?.length}<div class="skills" title={node.skills.join(', ')}>{node.skills.slice(0, 3).join(' · ')}</div>{/if}
...
<style>
  ...
  .run-state span:last-of-type { color: var(--voly-orange); border-color: color-mix(in srgb, var(--voly-orange) 55%, var(--border-default)); }
  .run-state button { padding: 2px 7px; border: 1px solid var(--accent-red); color: var(--accent-red); font: 9px var(--font-mono); text-transform: uppercase; }
  .run-state button:disabled { opacity: .5; }
  .live-steps { display: flex; flex-wrap: wrap; gap: 5px; }
  .live-step { display: inline-flex; align-items: center; gap: 5px; padding: 3px 7px; border: 1px solid var(--border-default); color: var(--text-muted); font: 9px var(--font-mono); text-transform: uppercase; }
  .live-step i { width: 6px; height: 6px; background: currentColor; }
  .live-step.status-running, .live-step.status-verifying { color: var(--voly-orange); border-color: var(--voly-orange); }
  .live-step.status-done, .live-step.status-completed, .live-step.status-verified { color: var(--accent-green); }
  .live-step.status-failed, .live-step.status-blocked { color: var(--accent-red); }
  ...
  .route, .skills, .metrics, .files, .error { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font: 10px var(--font-mono); color: var(--text-secondary); }
  .skills { color: var(--voly-orange); }

...
class TaskEvent:
    ...
    error: str | None = None
    # Error class of the final failed executor attempt (risk R4 drift metric):
    # "billing" | "not_available" | "timeout" | ErrorType.* | "unrecognized".
    # None on success. A growing "unrecognized" share means CLI output formats
    # drifted past the signal tables (see executor.base.classify_failure).
...
def summarize_error_classes(events: list[TaskEvent]) -> dict[str, Any]:
    """Failure breakdown by error class + share of unrecognized errors (R4).

    ``unrecognized_share`` is computed over failures that carry an error_class
    (events emitted before the field existed are counted as "(unclassified)"
    and excluded from the share, so old data doesn't skew the drift signal).
    """
    failed = [e for e in events if e.status == "failed"]
    by_class: dict[str, int] = {}
    for e in failed:
        key = e.error_class or "(unclassified)"
        by_class[key] = by_class.get(key, 0) + 1
    classified = sum(n for k, n in by_class.items() if k != "(unclassified)")
    unrecognized = by_class.get("unrecognized", 0)
    return {
        "total_events": len(events),
        "failed": len(failed),
        "by_class": by_class,
        "unrecognized_share": round(unrecognized / classified, 4) if classified else 0.0,
    }

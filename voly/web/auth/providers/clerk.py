"""Clerk session JWT provider (optional SSO).

Open-core note
--------------
SSO is **not** the open-core default. This provider is a **candidate for
extraction** into the private Team package ``voly-team`` (Option B split).
Do not import at package load unless ``auth.provider=clerk``.

Verification: PyJWT PyJWKClient (RS256) — no extra deps beyond voly[ui].
"""
...
_log = logging.getLogger("voly.web.auth.providers.clerk")
...
def decode_clerk_token(token: str, auth: AuthConfig) -> TokenPayload:
    """Validate a Clerk session JWT and return TokenPayload (sub = user id)."""
    if not token:
...
class ClerkProvider:
    """Optional Team SSO provider (extract target for voly-team)."""

    name = "clerk"

    def is_enforced(self, auth: AuthConfig) -> bool:
        return bool(auth.enabled and (auth.clerk_jwks_url or auth.clerk_issuer))

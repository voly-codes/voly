...
После установки используйте `voly quickstart --cwd ~/my-project`. Флаг `--yes`
создаёт отсутствующий `voly.yaml` без вопроса. Quickstart не устанавливает и не
запускает стороннего агента; первая предложенная команда использует `--dry-run`.

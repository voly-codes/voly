...
`Pipeline.run()` flow: `INIT → AGUI_START → A2A_DISCOVER → A2A_DELEGATE → ROUTE → MEMORY_RETRIEVE → RTK_FILTER → SKILL_INJECT → HEADROOM_COMPRESS → DSPY_PROGRAM_CALL → MODEL_CALL → MEMORY_STORE → AGUI_DONE → DONE / ERROR → emit TaskEvent`
...
| CLI command | `README.md`, `CLAUDE.md`, command-specific docs |
| Config/env | `README.md`, `codeops.yaml`, `.env.example` if needed |
| UI component | `docs/ARCHITECTURE.md` (ui/ section) |

## Do not commit

`.env`, `.codeops/events/`, `.codeops/dspy/datasets/`, `.codeops/dspy/programs/`, `.venv/`, `.pytest_cache/`, `.ruff_cache/`, `ui/node_modules/`, `codeops/web/static/assets/` (generated by build)

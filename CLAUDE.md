...
- **Retry-aware стоимость**: model-loops `zen`/`opencode` фолдят потраченное брошенными попытками в `ExecutorResult` (`_fold_retry_costs`); `AgentRunner` фолдит брошенные попытки цепочки в тоталы `TaskEvent` + поля `retry_count`/`retry_cost_usd`; `chain_timelog` несёт per-attempt cost/tokens; бюджет-чек по тоталу. `cost_usd` — всегда полный тотал (суммирование не задваивает). Тесты: `tests/test_retry_cost.py`; docs: `executors.md` («Retry-aware cost»). **Этап 1 завершён.**

**Ближайший фокус (ядро прежде монетизации):** этап 2 «Устойчивость цепочки» (heartbeat/watchdog поверх TaskEvent, локальный чекпоинтинг мульти-агента, git-чекпоинт перед ретраем) — перед стартом ответить на открытый вопрос №1 плана (реальная длина цепочек / частота зависаний).
...
voly dspy status            voly workflow list        voly workflow run
voly memory search <query>  voly serve                voly ui

...
def test_is_nested_a2a_request_from_env(monkeypatch) -> None:
    """Третье условие guard-а: env VOLY_A2A_NESTED=1 без маркеров в body."""
...
def test_is_nested_a2a_request_parent_wins_over_task_id() -> None:
    """При обоих маркерах контекст наследует явный a2a_parent_task_id."""
    nested, ctx = _is_nested_a2a_request(
        {"task": "do work", "task_id": "sub-1", "a2a_parent_task_id": "parent-9"}
    )
    assert nested is True
    assert ctx["a2a_parent_task_id"] == "parent-9"

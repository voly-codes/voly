...
def test_workflow_engine_start_unknown() -> None:
    ...
    with pytest.raises(ValueError, match="Unknown workflow"):
        engine.start("nonexistent-workflow")
...
def test_workflow_definition_validate_cycle_detection() -> None:
    ...
    wf.step("b", agent="dev", depends_on=["a"])
    errors = wf.validate()
...
class TestWorkflowClientErrors:
    def test_http_error(self) -> None:
        import urllib.error
        from unittest.mock import MagicMock, patch
        ...
        with patch("urllib.request.urlopen", side_effect=http_err):
            with pytest.raises(WorkflowClientError, match="HTTP 503"):
    ...
    def test_url_error(self) -> None:
        ...
        with patch("urllib.request.urlopen", side_effect=urllib.error.URLError(
            ...
            with pytest.raises(WorkflowClientError, match="Connection refused"):
                client.list_workflows()
